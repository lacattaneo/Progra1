...
def matriz(): #funcion para crear la matriz
    categorias=10
    meses=12
    matriz=[[0]*meses for i in range(categorias)]
    return matriz

def imprimir_matriz(matriz,cate,mes): #Funcion para imprimir matriz
    canCate = len(matriz)
...
def agregar_venta(matriz, cate, mes):
    while True:
        ...
        # Si el vendedor ingresa 0, se sale del bucle
        if cat_idx == -1:
        ...
        print(
            f"Se ha actualizado la categoría {cate[cat_idx]} para el mes {mes[mes_idx]} con {cantidad} juegos vendidos.")
        print()
